module alu #(parameter INPUT_WIDTH=4, CMD_WIDTH=4)(  
	input clk, rst, mode, ce,
	input [1:0] inp_valid,
	input [CMD_WIDTH-1:0] cmd,
	input [INPUT_WIDTH-1:0] ina, inb,
	input cin,
	...
	reg [2*INPUT_WIDTH-1:0] next_wait_res, next_res;
	...
	always @(*) begin
		if({mode,cmd} == MUL1) begin
			if(flag1) begin
				a = ina;
				b = inb;
			end 
		end else if ({mode,cmd} == MUL2) begin
			if(flag2) begin
				a = ina;
				b = inb;
			end 
		...
		case({mode, cmd})
			...
			SIGNED_ADD: begin
				if(inp_valid != 2'b11) 
					...
					next_oflow = (a[INPUT_WIDTH-1] == b[INPUT_WIDTH-1]) && (next_res[2*INPUT_WIDTH-1] != a[INPUT_WIDTH-1]);
					next_e = (a==b);
			...
			ROL_A_B: begin
				if(inp_valid != 2'b11) begin
					...
					next_err = (b>=$(1 << INPUT_WIDTH));
					next_res = a<<b[$clog2(INPUT_WIDTH):0];
			...
			ROR_A_B: begin
				if(inp_valid != 2'b11) begin
					...
					next_err = (b>$=clog2(INPUT_WIDTH)-1);
					next_res = a<<(INPUT_WIDTH-b[$clog2(INPUT_WIDTH):0]);
					next_res = {{INPUT_WIDTH{1'b0}}, next_res[INPUT_WIDTH-1:0]|next_res[2*INPUT_WIDTH-1:INPUT_WIDTH]};
				end
			end
		endcase
...
module tb;
    ...
    .cmd(cmd), .ina(opa), .inb(opb), .cin(cin),
    ...
    initial begin
        clk=0;rst=0;ce=1;inp_valid=2'b11;mode=1;cmd=9;cin=0;
		...
		@(negedge clk);
		opa=8; opb=9; mode=1;cmd=3;

		@(posedge clk);
        $display("%d", res);
		opa=8; opb=9; mode=1;cmd=5;
